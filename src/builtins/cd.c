...
{
	if (!ft_strcmp(cmd->args[1], "-") && !chdir(ft_env_get_val(*env, "OLDPWD")))
	{
		ft_env_update(env, "OLDPWD", current_dir);
		ft_env_update(env, "PWD", exec_pwd(env));
		return (printf("%s\n", ft_env_get_val(*env, "PWD")), EXIT_SUCCESS);
	}
	...
	else
	{
		...
		if (ft_strcmp(cmd->args[1], "-") == 0 && !ft_env_get_val(*env,
				"OLDPWD"))
			return (ft_putendl_fd("OLDPWD not set", 2), EXIT_FAILURE);
		else
			return (perror(cmd->args[1]), EXIT_FAILURE);
	}
}
